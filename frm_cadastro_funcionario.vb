Public Class frm_cadastro_funcionario
    ...
    Private Sub btn_gravar_Click(sender As Object, e As EventArgs) Handles btn_gravar.Click
        Try
            sql = "select * from  USUARIO where cpf='" & txt_cpf.Text & "'"
            rs = db.Execute(sql)
            If rs.EOF = False Then
                sql = "update USUARIO set data_nasc='" & cmb_calendario.Text & "', " &
                                                   "nome='" & txt_nome.Text & "'," &
                                                   "cep='" & txt_cep.Text & "'," &
            ...
            Else
                sql = "insert into USUARIO values ('" & txt_cpf.Text & "', " &
                                                   "'" & cmb_calendario.Text & "', " &
                                                   "'" & txt_nome.Text & "'," &
                                                   "'" & txt_cep.Text & "'," &
                                                   "'" & txt_endereco.Text & "'," &
    ...
    Private Sub txt_cpf_LostFocus(sender As Object, e As EventArgs) Handles txt_cpf.LostFocus
        Try
            ...
            Else
                ...
                txt_senha.Text = rs.Fields(12).Value
    ...
    Private Sub txt_buscar_nome_TextChanged(sender As Object, e As EventArgs) Handles txt_buscar_nome.TextChanged
        With dgv_dados
            cont = 1
            sql = "select * from USUARIO where nome like '" & txt_buscar_nome.Text & "%'"
            rs = db.Execute(sql)
            .Rows.Clear()
        ...
        Try
            With dgv_dados
                If .CurrentRow.Cells(4).Selected = True Then
                    ...
                    sql = "select * from USUARIO where cpf='" & aux & "' "
                    rs = db.Execute(sql)
                ...
                ElseIf .CurrentRow.Cells(5).Selected = True Then
                    ...
                    If resp = MsgBoxResult.Yes Then
                        sql = "delete from USUARIO where cpf ='" & aux & "'"
                        rs = db.Execute(sql)
                        carregar_dados_funcionario()
                    End If
                Else
